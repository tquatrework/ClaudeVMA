...
> Arbitrage d'architecture du 2026-08-06 (`docs/architecture.md` > "Arbitrages rendus") :
> `firstName`/`lastName`/`phone` appartiennent **exclusivement** à profile-service.
> identity-access-service ne les collecte plus du tout à la création de compte (retiré des DTO
> ci-dessous) et ne les rend donc plus obligatoires — il ne porte que l'authentification, les rôles
> et les consentements. Ce revirement succède à un état intermédiaire du 2026-08-05 où ces champs
> restaient collectés puis transmis à profile-service ; cet état intermédiaire est abandonné. Toute
> collecte de nom/prénom/téléphone (front, workflows d'onboarding) doit désormais alimenter
...
| POST | /accounts | Créer un compte générique (auto-inscription) | Non | — | `{email, password, role?, loginIdentifier?}` |
| POST | /accounts/students | Créer un compte élève (+ parent optionnel) | Non | — | `{email, password, loginIdentifier?, isMember?, parentLoginIdentifier?, parentEmail?, parentPassword?}` |
| POST | /accounts/teachers | Créer un compte formateur | Non | — | `{email, password, loginIdentifier?, cvReference?}` |
| POST | /accounts/parents | Créer un compte parent / financeur (+ élève optionnel) | Non | — | `{email, password, studentLoginIdentifier?, studentEmail?, studentPassword?}` |
...
Aucune des quatre routes de création de compte n'accepte `firstName`/`lastName`/`phoneNumber` —
la validation applique `whitelist: true` (ValidationPipe global), donc envoyer ces champs renvoie
`400` (champs non reconnus rejetés, jamais silencieusement ignorés).

**`POST /accounts/students` — élève + parent optionnel dans le même appel :**
- `parentLoginIdentifier` : lie un compte parent **existant** par identifiant (`404` si introuvable). Prioritaire sur `parentEmail`.
- `parentEmail` (sans `parentLoginIdentifier`) : `0` compte correspondant → crée un nouveau compte parent (sans profil administratif, à alimenter séparément côté profile-service ; `parentPassword` optionnel, retombe sur `password` sinon) ; `1` compte correspondant → lie ce compte existant ; `2+` comptes → `409` (utiliser `parentLoginIdentifier`).
- Élève et parent sont créés/liés dans **une seule transaction** : tout échec (parent introuvable, email ambigu, `503` profile-service lors de la liaison) annule l'élève ET le parent.
...
**`POST /accounts/parents` — parent + élève optionnel dans le même appel (symétrique du point ci-dessus) :**
- `studentLoginIdentifier` : lie un compte élève **existant** par identifiant (`404` si introuvable). Prioritaire sur `studentEmail`.
- `studentEmail` (sans `studentLoginIdentifier`) : `0` compte correspondant → crée un nouveau compte élève (sans profil administratif) ; `1` compte correspondant → lie ce compte existant ; `2+` comptes → `409` (utiliser `studentLoginIdentifier`).
...
Réponse (compte simple) : `{id, loginIdentifier, email, role, validationStatus, consentSigned, isActive, createdAt}` (`emailAlreadyUsed`/`suggestedLoginIdentifier` optionnels — **ne contient jamais `firstName`/`lastName`/`phone`**, propriété exclusive de profile-service).
...
`503 Service Unavailable` sur `POST /accounts/students` et `POST /accounts/parents` uniquement, lorsqu'un parent/élève est lié ou créé dans le même appel : profile-service indisponible ou en erreur lors de la liaison financeur/élève — la création de compte est **intégralement annulée** (transaction locale rollback), aucun compte orphelin n'est laissé en base. Le client peut réessayer l'appel tel quel.
...
Body `POST /internal/create-account` : `{email, password, role?, loginIdentifier?}` — mêmes règles de validation que `POST /accounts` (plus de `firstName`/`lastName`/`phoneNumber`, voir plus haut).
...
### Appel sortant vers profile-service (relation uniquement, aucune donnée d'identité)

> Arbitrage d'architecture du 2026-08-06 : identity-access-service ne collecte plus du tout
> `firstName`/`lastName`/`phone` (ni saisie, ni transmission à profile-service). L'unique appel
> sortant restant vers profile-service porte sur la relation finance-owner-student, sans aucune
> donnée d'identité.

Quand un élève et un parent financeur sont créés/liés dans le même appel (`POST /accounts/students`
avec `parentLoginIdentifier`/`parentEmail`, ou `POST /accounts/parents` avec
`studentLoginIdentifier`/`studentEmail`), identity-access-service appelle, **dans la même transaction
locale** que la création du ou des comptes : `POST /internal/link-parent` sur profile-service avec
...
Cet appel est **bloquant et obligatoire** : toute erreur (réseau, timeout 3s, HTTP non-2xx) fait
échouer toute la transaction locale — rollback du ou des comptes tout juste insérés — et la route
retourne `503` au client. Idempotence côté profile-service : `link-parent` est idempotent par paire
`(studentId, financeOwnerId)`.
...
| POST | /internal/create-administrative-profile | Créer/mettre à jour le profil administratif minimal d'un compte (upsert par `userId`) — `{userId, firstName, lastName, phone?}`. Non consommée par identity-access-service depuis l'arbitrage du 2026-08-06 (voir section identity-access-service) : identity-access-service ne collecte plus firstName/lastName/phone et n'a donc plus besoin de les transmettre. Champs `firstName`/`lastName`/`phone` inchangés côté profile-service (branche `refactor/consolidate-name-fields-ownership`) | `X-Internal-Secret` |
