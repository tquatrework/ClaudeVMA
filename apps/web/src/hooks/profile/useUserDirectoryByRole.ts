...
export async function loadUserDirectoryByRole(role: UserRole): Promise<UserDirectoryResult> {
  const collectedEntries: UserDirectoryEntry[] = []
  let currentPage = 1
  let totalPages = 1

  while (currentPage <= totalPages && currentPage <= MAX_USER_DIRECTORY_PAGES) {
    const response = await fetchUserDirectoryByRole(role, currentPage, USER_DIRECTORY_PAGE_SIZE)
...
export function useUserDirectoryByRole(
  ...
  const { data, isLoading, error } = useAsyncData(
    () => (isEnabled ? loadUserDirectoryByRole(role) : pendingForever<UserDirectoryResult>()),
    [role, isEnabled],
