...
describe('AdminService', () => {
  ...
  describe('logAdminAction()', () => {
    ...
    it('AOS-BR-002: nom AF présent n\'importe où dans la chaîne d\'action', async () => {
      await expect(
        adminService.logAdminAction(
          TI_ID,
          'technicien_informatique',
          'update.role.to.administrateur_financier.for.user',
          'user',
          'user-002',
          'Attribution AF au milieu de la chaîne',
        ),
      ).rejects.toThrow(ForbiddenException);
    });

    it('passe correlationId et ipAddress au repository', async () => {
      const savedLog = {
        id: 'log-0003',
        operatorId: TI_ID,
        operatorRole: 'technicien_informatique',
        ...
        resourceType: 'user',
        resourceId: 'user-003',
        details: 'Verrouillage de compte',
      ...
      await adminService.logAdminAction(
        TI_ID, 'technicien_informatique', 'user.account.locked',
        'user', 'user-003', 'Verrouillage de compte', 'corr-xyz', '192.168.1.1',
      );

      expect(activityLogRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ correlationId: 'corr-xyz', ipAddress: '192.168.1.1' }),
      );
    });
  });

  // ─────────────────────────────────────────────────────────────────────────
  // findActivityLogs() — filtres et pagination supplémentaires
  // ─────────────────────────────────────────────────────────────────────────

  describe('findActivityLogs() — filtres avancés', () => {
    it('filtre par resourceId', async () => {
      activityLogRepo.findAndCount.mockResolvedValue([[], 0]);

      await adminService.findActivityLogs(
        { resourceId: 'forum-007' },
        'technicien_informatique',
      );

      expect(activityLogRepo.findAndCount).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ resourceId: 'forum-007' }),
        }),
      );
    });

    it('applique la pagination page > 1', async () => {
      activityLogRepo.findAndCount.mockResolvedValue([[], 0]);
    ...
    it('utilise limit personnalisé', async () => {
      activityLogRepo.findAndCount.mockResolvedValue([[], 0]);
    ...
    it('lève ForbiddenException pour le rôle parent_financeur', async () => {
      await expect(
        adminService.findActivityLogs({}, 'parent_financeur'),
      ).rejects.toThrow(ForbiddenException);
    });
  });

  // ─────────────────────────────────────────────────────────────────────────
  // findTechnicalLogs() — filtres avancés
  // ─────────────────────────────────────────────────────────────────────────

  describe('findTechnicalLogs() — filtres avancés', () => {
    it('filtre par service', async () => {
      technicalLogRepo.findAndCount.mockResolvedValue([[], 0]);
      ...
      expect(technicalLogRepo.findAndCount).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ service: 'identity-access-service' }),
        }),
      );
    });
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // createVisibilityOverride() — stockage des champs
  // ─────────────────────────────────────────────────────────────────────────

  describe('createVisibilityOverride() — stockage des champs', () => {
    it('stocke createdById et createdByRole correctement', async () => {
      visibilityOverrideRepo.findOne.mockResolvedValue(null);
      const savedOverride = buildSampleOverride({ createdById: TI_ID, createdByRole: 'technicien_informatique' });
      visibilityOverrideRepo.create.mockReturnValue(savedOverride);
      visibilityOverrideRepo.save.mockResolvedValue(savedOverride);

      await adminService.createVisibilityOverride(
        { resourceType: 'forum', resourceId: 'forum-002', reason: 'test' },
        TI_ID,
        'technicien_informatique',
      );

      expect(visibilityOverrideRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ createdById: TI_ID, createdByRole: 'technicien_informatique' }),
      );
    });
  });

  // ─────────────────────────────────────────────────────────────────────────
  // liftVisibilityOverride() — champs liftedAt/liftedById
  // ─────────────────────────────────────────────────────────────────────────

  describe('liftVisibilityOverride() — champs de levée', () => {
    it('définit liftedById et liftedAt lors du lift', async () => {
      const activeOverride = buildSampleOverride({ isActive: true });
      visibilityOverrideRepo.findOne.mockResolvedValue(activeOverride);
      visibilityOverrideRepo.save.mockImplementation((data) => Promise.resolve(data));
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // updateSiteMetadata() — branche description optionnelle
  // ─────────────────────────────────────────────────────────────────────────

  describe('updateSiteMetadata() — description optionnelle', () => {
    it('met à jour description si elle est fournie', async () => {
      const existingMetadata = buildSampleMetadata();
      siteMetadataRepo.findOne.mockResolvedValue(existingMetadata);
      siteMetadataRepo.save.mockImplementation((data) => Promise.resolve(data));
      ...
      await adminService.updateSiteMetadata(
        METADATA_ID,
        { metaValue: 'https://new.visiomaths.fr', description: 'Nouvelle description' },
        TI_ID,
        'technicien_informatique',
      );
    ...
    it('ne touche pas description si elle est undefined', async () => {
      const existingMetadata = buildSampleMetadata({ description: 'Description originale' });
      siteMetadataRepo.findOne.mockResolvedValue(existingMetadata);
      siteMetadataRepo.save.mockImplementation((data) => Promise.resolve(data));
      ...
      expect(siteMetadataRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ description: 'Description originale' }),
      );
    });
  });

  // ─────────────────────────────────────────────────────────────────────────
  // getHealthMetrics() — rôles supplémentaires
  // ─────────────────────────────────────────────────────────────────────────

  describe('getHealthMetrics() — rôles supplémentaires', () => {
    it('lève ForbiddenException pour le rôle parent_financeur', async () => {
      await expect(
        adminService.getHealthMetrics('parent_financeur'),
      ).rejects.toThrow(ForbiddenException);
    });

    it('lève ForbiddenException pour le rôle eleve', async () => {
      await expect(
