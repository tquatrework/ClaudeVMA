 ...
 * - `post` : séquence ordonnée de blocs `title`/`text`/`image`, `videoUrl` interdit.
 *
 ...
 * (`docs/services/content-catalog-service.md`, entité `TutorialBlock`) : `content` (texte brut,
 * syntaxe légère `$...$`/`$$...$$`/`[label](url)` rendue côté client uniquement) pour
 * `title`/`text`, champs `image*` pour `image`. Mêmes colonnes et même mécanisme de stockage que
...
export type TutorialBlockCategory = 'title' | 'text' | 'image'
...
/**
 * `content` requis pour `category: 'title'|'text'`, optionnel (légende) pour `category: 'image'`.
