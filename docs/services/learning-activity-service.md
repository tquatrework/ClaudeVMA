  ...
  <implementationSession date="2026-09-01" topic="Refonte des Evaluations - tentatives, correction manuelle, notifications">
    ...
    <folderStructure>
      ...
      <folder path="src/evaluation-attempts/">
        ...
        <file path="profile-relations-client.service.ts">Client HTTP interne vers profile-service (X-Internal-Secret) : GET /internal/relations/teachers/:studentId → {studentId, teacherUserIds}. Contrat confirmé le 2026-09-01 contre la route réelle livrée par profile-service (PR #197) — corrige une première implémentation construite sur l'hypothèse non confirmée {teacherIds}, qui provoquait un 502 systématique sur POST /evaluation-attempts/:id/request-correction. Un 404 amont est traité comme liste vide, jamais comme une erreur bloquante</file>
    ...
    <pendingPoints>
      <item>RÉSOLU le 2026-09-01 : contrat GET /internal/relations/teachers/:studentId côté profile-service confirmé contre la route réelle livrée (PR #197) — le champ s'appelle teacherUserIds, pas teacherIds. La première implémentation avait construit le client sur l'hypothèse non confirmée {teacherIds}, provoquant un 502 "Réponse de relations malformée (profile-service)" systématique sur POST /evaluation-attempts/:id/request-correction. Corrigé sur la branche fix/learning-activity-teacher-relations-field-name. Un 404 amont reste absorbé en liste vide (comportement défensif inchangé, pour ne pas bloquer une demande de correction d'un élève sans professeur lié).</item>
