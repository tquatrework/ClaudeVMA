...
<serviceFunctionalSpecification version="1.0" source="Cahier des Charges VisioMath - V 1.1.1 - 240531.docx" previousSource="CdC VisioMath - simplifie.docx" status="completed-from-integral-cdc">
  ...
  <implementationNotes>
    ...
    <session date="2026-08-05">
      ...
      <supersededNote date="2026-08-06">
        Les décisions S-profile-service-sole-owner, S-mandatory-profile-write-with-rollback et
        S-parent-combined-creation-and-auto-link ci-dessus sont partiellement revirées par la
        session du 2026-08-06 ci-dessous : identity-access-service ne collecte plus du tout
        firstName/lastName/phone (au lieu de les collecter puis les transmettre en écriture
        primaire bloquante à profile-service). La suppression locale des colonnes (migration,
        entité) et le support combiné parent/élève avec liaison automatique
        (linkParentToStudent) restent en revanche inchangés — ce sont deux préoccupations
        indépendantes de la propriété des champs d'identité.
      </supersededNote>
    </session>

    <session date="2026-08-06">
      <title>Revirement : identity-access-service ne collecte plus firstName/lastName/phone (au lieu de les transmettre à profile-service)</title>
      <context>
        Arbitrage d'architecture rendu le 2026-08-06 (docs/architecture.md > "Arbitrages rendus"),
        après plusieurs allers-retours divergents entre le 4 et le 5 août 2026 sur des branches non
        fusionnées : firstName/lastName/phone appartiennent exclusivement à profile-service.
        identity-access-service ne doit plus les stocker (déjà acquis depuis la session du
        2026-08-05) ni les rendre obligatoires ni même les collecter à la création de compte — il
        ne porte que l'authentification, les rôles et les consentements. Toute collecte de
        nom/prénom/téléphone (front, workflows d'onboarding) doit désormais alimenter
        profile-service directement, jamais transiter par identity-access-service.
        Repris à partir des 4 commits de `feat/identity-access-profile-sync-and-auto-link`
        (session du 2026-08-05, non fusionnée) rebasés sur master à jour (inclut #54-#58) : la
        suppression des colonnes locales (migration, entité) et le support combiné
        élève+parent/parent+élève avec liaison automatique finance-owner-student sont conservés
        tels quels — seule la collecte et la transmission de firstName/lastName/phone est retirée.
      </context>

      <decision id="S-no-name-collection-in-identity-access">
        <title>Retrait complet de firstName/lastName/phoneNumber des 4 DTO de création de compte</title>
        <files>
          <file>src/accounts/dto/create-account.dto.ts</file>
          <file>src/accounts/dto/create-student-account.dto.ts</file>
          <file>src/accounts/dto/create-teacher-account.dto.ts</file>
          <file>src/accounts/dto/create-parent-account.dto.ts</file>
          <file>src/accounts/dto/phone-number.validator.ts (supprimé)</file>
        </files>
        <description>
          firstName, lastName, phoneNumber (et les champs conditionnels parentFirstName/
          parentLastName sur CreateStudentAccountDto, studentFirstName/studentLastName sur
          CreateParentAccountDto) sont retirés des 4 DTO — non plus seulement "non persistés",
          mais totalement absents du contrat. Le ValidationPipe global (`whitelist: true`, voir
          main.ts) rejette désormais ces champs avec 400 s'ils sont envoyés (whitelist strips puis
          la validation de forme des champs restants s'applique normalement — voir test e2e
          dédié). phone-number.validator.ts (PHONE_NUMBER_REGEX) supprimé : plus aucun DTO ne
          valide de numéro de téléphone. Impact caller : orchestration-service (PR #57) et le
          front (PR #58) avaient été mis à jour pour envoyer firstName/lastName sur ces routes —
          ce revirement casse ce contrat, à répercuter dans ces deux services (hors périmètre de
          cette session, signalé à l'orchestrateur).
        </description>
        <status>resolved</status>
      </decision>

      <decision id="S-remove-profile-write-on-creation">
        <title>Suppression de l'écriture primaire vers profile-service à la création de compte</title>
        <files>
          <file>src/accounts/accounts.service.ts</file>
          <file>src/common/clients/profile-service.client.ts</file>
        </files>
        <description>
          persistAdministrativeProfile() et ProfileServiceClient.createAdministrativeProfile()
          (POST /internal/create-administrative-profile) sont supprimés : il n'y a plus rien à
          transmettre. createAccount et createTeacherAccount reviennent à une écriture simple sans
          DataSource.transaction (comme avant la session du 2026-08-05 : un save() unique est déjà
          atomique). createStudentAccount et createParentAccount conservent leur
          DataSource.transaction (nécessaire pour l'atomicité élève+parent et la liaison
          automatique, indépendante des champs d'identité). ProfileServiceClient ne porte donc plus
          que linkParentToStudent (relation finance-owner-student) — aucune méthode de lecture
          n'a été ajoutée : le contrat interne de profile-service (docs/routes.md) n'expose pas de
          route GET pour lire firstName/lastName/phone, et aucun consommateur restant
          d'identity-access-service n'en a besoin (voir décision suivante).
        </description>
        <status>resolved</status>
      </decision>

      <decision id="S-teacher-notification-uses-email">
        <title>Notification dashboard formateur : email au lieu de firstName/lastName</title>
        <files>
          <file>src/accounts/accounts.service.ts</file>
        </files>
        <description>
          notifyDashboardTeacherPending() (notification non-bloquante vers
          dashboard-notification-service à la création d'un compte formateur) utilisait
          firstName/lastName du DTO pour construire un nom lisible. Ces champs n'existant plus,
          la notification transmet désormais l'email du formateur (teacherEmail) — disponible
          sans appel réseau supplémentaire, contrairement à une lecture via ProfileServiceClient
          qui aurait ajouté une dépendance et un risque de course (le profil administratif du
          formateur n'existe pas forcément encore côté profile-service au moment de cette
          notification, puisqu'il n'est plus créé par cet appel).
        </description>
        <status>resolved</status>
      </decision>

      <decision id="S-mail-service-unchanged">
        <title>MailService : aucun changement nécessaire (déjà générique depuis le 2026-08-05)</title>
        <description>
          sendEmailVerification/sendPasswordReset avaient déjà perdu leur paramètre firstName lors
          de la session du 2026-08-05 (salutation générique). Aucune régression, aucun changement
          nécessaire cette session — confirmé par lecture du fichier avant modification.
        </description>
        <status>resolved</status>
      </decision>

      <openItem id="TD-caller-contract-break">
        <title>Rupture de contrat pour les appelants existants (orchestration-service, front)</title>
        <description>
          orchestration-service (PR #57, "propager prénom/nom dans les workflows d'onboarding") et
          le front (PR #58, "rendre prenom/nom obligatoires a l'inscription parent") envoient
          actuellement firstName/lastName sur les routes de création de compte
          d'identity-access-service. Depuis cette session, ces champs sont rejetés (400,
          whitelist). Signalé à l'orchestrateur — la mise en cohérence de ces deux services (ne
          plus envoyer firstName/lastName à identity-access-service, les envoyer directement à
          profile-service) est hors périmètre de cette session (dossier de travail limité à
          identity-access-service).
        </description>
        <status>open</status>
      </openItem>

      <openItem id="TD-e2e-not-executed-in-session" inheritedFrom="2026-08-05">
        <title>Suite e2e (test/app.e2e-spec.ts) non exécutée contre une vraie base dans cet environnement</title>
        <description>
          Toujours d'actualité : test/app.e2e-spec.ts nécessite une base Postgres réelle
          (DATABASE_URL) et n'est matché par aucun script npm existant (test:e2e ne matche que
          test/e2e/**). Tentative de démarrer un Postgres via Docker dans cet environnement :
          `docker pull postgres:16-alpine` échoue avec la même erreur qu'en session précédente
          (cache local containerd corrompu, blob introuvable), aucun Postgres local en écoute sur
          5432. Validation effectuée à la place : suite unitaire complète (196/196 tests verts,
          incluant les scénarios de rollback transactionnel et de liaison automatique) +
          `tsc --noEmit` sans erreur sur l'ensemble du projet (src + test, y compris
          test/app.e2e-spec.ts, vérifié via un tsconfig temporaire incluant test/**) + `nest build`
          réussi. Le fichier e2e a été mis à jour (retrait des champs d'identité des payloads,
          nouveau scénario whitelist 400, stub ProfileServiceClient réduit à linkParentToStudent)
          mais reste non exécuté contre une vraie base — à valider avant prochain déploiement ou
          dans un environnement doté d'un Postgres accessible.
        </description>
        <status>open</status>
      </openItem>
    </session>
